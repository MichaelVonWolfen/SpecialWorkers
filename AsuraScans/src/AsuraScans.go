...
func processData(rawStr string) []map[string]interface{} {
	...
	index := -1
	organizedData := make([]map[string]interface{}, 0)
	for _, item := range extracted {
		for k := range item {
			if k == "status" {
				index += 1
				organizedData = append(organizedData, item)
			} else {
				organizedData[index][k] = item[k]
			}
		}
	}
