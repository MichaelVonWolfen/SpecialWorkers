...
func worker(data models.WorkerInformation) ([]models.MangaInformation, error) {
	...
	//Call ProcessData function to get the manhwa data
	//access all the manga url's
	return nil, nil
}
						...
						if props, ok := n[3].(map[string]interface{}); ok {
							if val, ok := props["children"]; ok {
								if children, ok := val.(string); ok {
									...
									if slices.Contains([]string{"Ongoing", "Completed", "Hiatus", "Cancelled", "Unknown", "Dropped"}, children) {
...
func processData(rawStr string) []models.MangaInformation {
	str := preProcess(rawStr)
	var root interface{}
	if err := json.Unmarshal([]byte(str), &root); err != nil {
		log.Printf("Failed to parse JSON: %v", err)
		return []models.MangaInformation{}
	}
	...
	index := -1
	var organizedData []models.MangaInformation
	for _, item := range extracted {
		for k := range item {
			switch k {
			case "status":
				index += 1
				organizedData = append(organizedData, models.MangaInformation{
					Status:  item[k].(string),
					MangaId: -1,
				})
				break
			case "title":
				organizedData[index].MangaName = item[k].(string)
				break
			case "url":
				organizedData[index].MangaUrl = item[k].(string)
				break
			}
		}
	}
